...
const ChatWidget = ({ topic = "" }) => {
  ...
  const showBotDoubtText = useRef(true);

  const handleExpand = () => {
    setExpanded(expanded === 'true' ? 'false' : 'true');
    setNotificationCount(0);
    setShowTooltip("false");
    showBotDoubtText.current = false;
  }
  ...
  return (
    <>
      {expanded === "false" &&
      <div className="chatbot-container text-black" onClick={expanded === 'false' ? handleExpand : undefined} role='button'>
        {showBotDoubtText.current && 
          <div className="chatbot-text">
            Have doubts? I can help you?
          </div>
        }
              ...
              <ChatArea>
                <MessageList>
                  {messages.map(msg => (
                    <MessageItem key={msg.id} isownmessage={msg.isOwnMessage}>
                      <div className='d-flex align-items-center justify-content-between' style={{marginBottom: '5px' }}>
                        <Avatar isUser={msg.isOwnMessage === 'true'} className={`${msg.isOwnMessage === 'true' ? 'text-white' : 'text-black'}`}>
                          {msg.isOwnMessage === 'true' ? <i className='fas fa-user' /> : <Logo src={logo} alt="AI Guru" width={15} height={15}/>}
                          &nbsp; &nbsp;
                          <i>{msg.sender}</i>
                        </Avatar>
                  ...
                  {isTyping && (
                    <TypingIndicator>
                      <Avatar isUser={false}>
                        <Logo src={logo} alt="AI Guru" width={15} height={15}/>
                      </Avatar>
                      <TypingDot delay={0} />
                      <TypingDot delay={0.2} />
                      <TypingDot delay={0.4} />
